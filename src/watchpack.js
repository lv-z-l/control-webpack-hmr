...
var DecrateWatchpack = require('./DecrateWatchpack.js')
...
Watchpack.prototype._fileWatcher = function _fileWatcher(file, watcher) {
  watcher.on(
    'change',
    function (mtime, type) {
      this.onChange(file, mtime, file, type)
    }.bind(this)
  )
  watcher.on(
    'remove',
    function (type) {
      this.onRemove(file, file, type)
    }.bind(this)
  )
  return watcher
}
...
Watchpack.prototype._dirWatcher = function _dirWatcher(item, watcher) {
  watcher.on(
    ...
    function (file, mtime, type) {
      this.onChange(item, mtime, file, type)
    }.bind(this)
  )
  return watcher
}
